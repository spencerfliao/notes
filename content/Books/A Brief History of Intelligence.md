---
Type: book
Subtitle: Evolution, AI, and the Five Breakthroughs That Made Our Brains
Cover: https://i.gr-assets.com/images/S/compressed.photo.goodreads.com/books/1678219676l/62050269.jpg
Author: Max Bennett
Year: 2022
Topic:
  - lifesci
...
Graphs by Rebecca Gelernter, Mesa Schumacher, and Max Bennett
...
* ability to learn associations
	* between stimuli (bell & food) or action & consequence (lever & food)
	* unlimited: ascribe value to non-reflex-inducing stimulus/action
...
* invariance problem: different angles/pitches
	* hierarchy of layers of increasing receptive field size processing increasingly wholistic and complex patterns
...
###### internal model
* perceives its own direction in hindbrain (vestibular sense)
* perceives 3d space in hippocampus (place cells for spatial maps)
...
* filling in + one at a time + cannot be unperceived
* inference: we don’t perceive what is actually experienced, we perceive a simulated reality inferred from what we experience
...
* humans optimize for how well simulated reality predicts external sensory inputs
* Helmholtz machine
	* wake phase
		* recognition network observes input into hidden states, and from them  generation network reconstructs into output
		* diff between output and input backpropagated through both networks
	* sleep phase
		* generation from hidden state first, then recognization of the output into new hidden states
		* diff between old and new hidden states backpropagated
	* wake phase only: auto-encoders; sleep phase comparable to imagining/dreaming
	* unsupervised, but learns to both generalize pattern & generate novel examples of those patterns

###### hallucinate, dream, imagine
- hallucination is an inherent property of gen models
	- with sensory input: perception is constrained hallucination
	- without sensory input: unconstrained hallucination
	- e.g. Charles Bonnet syndrome: no signal, start "seeing" a lot
- balance between recognition & generation
	- wake: unbalanced amount of recognition
	- REM sleep: stabilizing & recalibrate through generation = dreaming
		- specifically REM deprived → more hallucinations
		- no recalibration = not just fatigue, but generation actively overrides input
- imagination = generation (not obvious, could have been separate)
	- cannot simultaneously recognize & imagine - pseudo-blind
	- employs same neurons for same subject matters
	- e.g. record brain activity → decode dream

###### evidence that perception is gen model: all above
- filling in + one at a time + cannot be unperceived
- wiring of neocortex, properties consistent with gen models
- inseparability of perception and imagination
- why we hallucinate dream

###### predicting the world
| early bilaterians         | early vertebrates | early mammals      |
| ------------------------- | ----------------- | ------------------ |
| reflex circuits           | basal ganglia     | neocortex          |
| predict reflex activation | predict rewards   | predict everything |
- world model
	- (above) inference: we don’t perceive what is actually experienced, we perceive a simulated reality inferred from what we experience
	- inferring/explaining causes—the rendered 3D world model—that could match sensory inputs
	- decoupled from external world: simulation can be independently explored
		- relive past events
		- predict consequences of actions never taken
- VS genAI: human-like symbolic language, but missing world model
- benefits
	- brains before could already recognize well
	- unlocks **3 new abilities (below)**

###### new abilities with simulation
1. vicarious trial & error
	- e.g. rats head toggling behavior at forks in mazes where choices ≠ obvious
		- probes: [[Hippocampus]] activated NOT for actual location but for possible future paths
		- playing out each option to plan ahead
	- e.g. rats run to salty food when salt-deprived though previously avoided
		- past rewards not predictive of current rewards
		- only possible if each path was simulated
	- both not performed by non-mammals
2. counterfactual learning
	- flaw of early vertebrate RL: only reinforces actions *actually* taken
		- small subset of possible paths *could have been* taken
	- counterfactual = what if made different choice in the past
		- regret, ruminating about what could have been
		- useful because same situation may recur
		- e.g. monkey rock-paper-scissors biased to the move that would have won (≠ temporal diff learning)
	- causation = what would have happened if things were different
		- if X then Y, if no X then no Y: causal
		- without counterfactual: only correlation
	- credit assignment up to now
		- early bilaterians: blocking, latent inhibition, overshadowing
		- early vertebrates: critic predicts changes in future rewards
		- early mammals: counterfactual & causation
3. episodic memory
	- memory = simulating a past reality (imagining = simulating future reality)
		- NOT truly remembering episodic events
		- similar neural circuitry as imagining the future
		- filled in, less accurate than we believe (77% exonerated because of mistaken eyewitness testimony)
		- imagining an event increases confidence that the event occurred
	- process
		- [[Hippocampus]] (quickly learn patterns) + neocortex (render world model)
		- HP repurposed for encoding new memory because it needs to be stored rapidly
		- activation of simulations retrieved by reactivating HP corresponding pattern


###### simulation VS model-free

| temporal difference learning                                                                 | model-based RL                                                                                          |
| -------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| early vertebrates                                                                            | early mammals                                                                                           |
| learns associations between current states and best actions                                  | learns a model of causations used for simulating actions                                                |
| faster but less flexible                                                                     | slower but more flexible                                                                                |
| no neocortex                                                                                 | neocortex required                                                                                      |
| straightforward rewards                                                                      | moves that aren't great now but set you up well for the future                                          |
| e.g. habitually going to work by responding to cues (traffic light, landmark) as it comes up | e.g. considering different ways to get to work and picking what you imagined gets you there the fastest |
- the search problem
	- possible actions are infinite, needs prioritization
	- **solution: verify & expand on "hunches" of basal ganglia actor-critic system**
	  e.g. AlphaGo:, simulated future possibilities

###### aPFC triggers simulation
- agranular PFC - motor cortex - granular PFC, old to new
	- aPFC refers to that of early mammals (or rodents), where the neocortex is exclusively agranular
- simulation in sensory cortex
- process
	- triggers simulation: when, what, how detailed of rendering
	- damage = no vicarious trial & error, episodic memory recall, counterfactual learning, impulse control
	- input: HP (processes sequence of places), hypothalamus (valence), [[Amygdala]] (affective states)
- e.g. stroke to aPFC (akinetic mutism): functioning perception, but "empty", nothing "mattered" (no simulation triggered)

###### intent decides behavior
- aPFC creates intent = models/predicts itself to explain/decide behavior

| frontal neocortex                                                                  | sensory neocortex                              |
| ---------------------------------------------------------------------------------- | ---------------------------------------------- |
| self model                                                                         | world model                                    |
| input: HP, HTH, amygdala                                                           | input: sensory organs                          |
| explain/predict the behavior of itself                                             | explain/predict the behavior of external world |
| "I'm *drinking* because I'm thirsty"<br>"I *should drink* because I'll be thirsty" | "I *see* this because there is water"          |
- active inference ≈ explaining sensory info, but produces behavior
- advantages
	- adapt for variable environments
	- forward thinking
		- anticipating future needs
		- inhibition & self-control
	- helps choose & narrow down what to simulate

###### decision process
1. uncertainty triggers simulation
	1. aPFC most excited when wrong/unexpected
	2. degree of disagreement of prediction in aPFC columns / actor-critics = uncertainty
	   e.g. divergence in ensemble in machine learning
2. simulation
	1. aPFC modulates sensory cortex = send instructions for specific simulations
	2. aPDF evaluates whether to continue / explore other
	3. "showing" rather than "telling"
3. choosing an option
	1. "votes" to suggest action
	2. vicarious trained: it doesn't discriminate between real and simulated worlds

###### habits = intents no longer
- repeated sufficient times
- creates habit: automated motor response triggered by sensory cues, model-free
- basal ganglia took over, no aPFC considering consequences, detached from high-level goal
- saves times & energy, complete complex behaviors easily
  e.g. Thinking, Fast and Slow: system 2 (slow)

###### self-control by PFC
- planning (all above)
- attention = maintain simulation despite distractions
- working memory = maintain simulation after sensory cues are gone
- inhibit amygdala & BG: prevent impulsive responses
	- e.g. tired/stressed → less energy for inhibition

###### motor cortex
- map of body mirroring somatosensory cortex (proportional to skill)
- function: motor planning
	- simulate voluntary, complex, coordinated sequences of movements ahead of time
	- precision↑ by integrating sensory & motor info
	- imagining ≈ doing, e.g. mental rehearsal improves maneuvers
- involuntary: reflex, autonomic, respiratory

###### motor hierarchy

|                | high-level goal                              | low-level goal                           |
| -------------- | -------------------------------------------- | ---------------------------------------- |
| **simulation** | **PFC**<br>plans, navigation                 | **motor cortex**<br>fine motor movements |
| **automation** | **BG front**<br>habitual cravings, addiction | **BG back**<br>habitual motor response   |
- distributing effort across different functions
- e.g. “utilization behavior”, “field-dependent behavior”: broken hierarchy

### 4 Breakthrough: Mentalization

frugivore
- ecological niche: abundance of energy and time → politick/gossip
- requires anticipation of ripening & future hunger
- explains primate brain size variation more than social group size

model mental state dissociated from your own
- anticipate future needs
- theory of mind (arms race of deception & counter-deception in politick/gossip)
- imitation learning (repurposed from politick/gossip)

granular PFC project self into simulation
- input from aPFC - modeling one's own mind's desire & thinking
- model other minds

simulate others' movements ≈ understand intentions (same areas)

acquiring new skills through observation requires theory of mind
- enables active teaching
- stay focused over long periods
- tell intent from non-intentional apart

### 5 Breakthrough: Language

human VS other apes
- individual: for most capabilities, unique in scale, not type
- triggers societal change: shared simulation across strangers/generations → hive-mind
- evolution of ideas

uniqueness of language: symbolic + grammatical
- detail & flexibility of communication
- transfer & render the same inner simulation
- expands source of learning: others' actions, real or imagined
- collective subjectivity → collaboration between large number of strangers
- transfer & accumulation of information
...
- old: emotional expressions: laugh, hoot, squeal
- new: language emerged from specific subsystems
	- independent skill ≠ more neocortex
	- network of regions work together ≠ language organ

curriculum
- too info-dense to code into genome
- generic learning system (cortex) + unique curriculum (instinct)
- tethering symbols to components in inner simulation
	- joint attention
	- turn-taking proto-conversation
	- assign labels
	- ask questions
- VS machine learning: sequence prediction

homo erectus adaptions
- cooking invented → bigger brain, more energy, smaller jaw, shorter digestive track
- bigger brain → premature birth → promiscuous mating to monogamous pair-bonding
- grandmothering: menopause → supporting children's children
- 50w YA: complex-language-ready vocal cords
- 20w YA: MRCA of modern humans spoke complex language

reinforcing cycle of moral society
- more gossip → altruism & punishment → language to share info → ...

LLM chain-of-thought reasoning
- exhibit emergent properties of thinking
- but still without human-like simulation
...
6. ensemble divergence & uncertainty
...
* do not delineate clearly and span all supposed layers
...
independently evolved simulations & episodic memory
- mammals
- birds
- cephalopods
...
- is there really developmental atrophy of layer 4 in agranular PFC because it has no need for it?
	- agranular PFC should refer to that of early mammals (or rodents), where the neocortex is exclusively agranular
	- granular PFC developed layer 4 later in early primates & took on the role
...
What is most striking when we examine the brains of other animals is how remarkably similar their brains are to our own. The difference between our brain and a chimpanzee’s brain, besides size, is barely anything.

survival instincts, emotions, and cognition do not delineate cleanly—they emerge from diverse networks of systems spanning all three of these supposed layers.

I wrote this book because I wanted to read this book.

### 1: The World Before Brains

The fungal strategy was, by some measures, more successful than the animals’—by biomass, there is about six times more fungus on Earth than animals. But as we will continually see, it is usually the worse strategy, the harder strategy, from which innovation emerges.
...
But it may not be a coincidence that the first successful domestic robot contained an intelligence not so unlike the intelligence of the first brains. Both used tricks that enabled them to navigate a complex world without actually understanding or modeling that world.
...
spending energy escaping is worth the cost only if the stimulus is in fact escapable. Otherwise, the worm is more likely to survive if it conserves energy by waiting.
...
It seems that at the same time valence—the categorizing of things in the world into good and bad—emerged, so too did the ability to use experience to change what is considered good and bad in the first place.

An early bilaterian that could remember to avoid a chemical that had previously been found near predators would survive far better than a bilaterian that could not.

broken associations are rapidly suppressed but not, in fact, unlearned; given enough time, they reemerge.
...
The second trick was overshadowing. When animals have multiple predictive cues to use, their brains tend to pick the cues that are the strongest—strong cues overshadow weak cues.

The third trick was latent inhibition—stimuli that animals regularly experienced in the past are inhibited from making future associations. In other words, frequent stimuli are flagged as irrelevant background noise. Latent inhibition is a clever way to ask, “What was different this time?”
...
At first, dopamine neurons did respond like a valence signal, getting uniquely excited whenever a hungry monkey got sugar water. But after a few trials, dopamine neurons stopped responding to the reward itself and instead responded only to the predictive cue.
...
This trick is called auto-association; neurons in the cortex automatically learn associations with themselves. This offers a solution to the generalization problem—the cortex can recognize a pattern that is similar but not the same.
...
While V1 decomposes pictures into simple features, as visual information flows up the hierarchy, it is pieced back together into whole objects.
...
Even though there are no explicit rewards until you get past all the rooms in level one, these AI systems didn’t need any external rewards to explore. They were motivated on their own. Simply finding their way to a new room was valuable in and of itself.
...
The electrical signaling of neurons is highly sensitive to temperature—at lower temperatures, neurons fire much more slowly than at warmer temperatures. This meant that a side effect of warm-bloodedness was that mammal brains could operate much faster than fish or reptile brains.

#### 11: Generative Models And The Neocortical Mystery

According to Mountcastle, the neocortex does not do different things; each neocortical column does exactly the same thing. The only difference between regions of neocortex is the input they receive and where they send their output; the actual computations of the neocortex itself are identical.
...
might optimize for the accuracy with which the inner simulated reality predicts the current external sensory input.
...
How groundbreaking this was cannot be overstated—neuroscientists were peering directly into the brain of a rat, and directly observing the rat considering alternative futures.
...
we don’t truly remember episodic events. The process of episodic remembering is one of simulating an approximate re-creation of the past.
...
The primary input to the agranular prefrontal cortex, however, comes from the hippocampus, hypothalamus, and amygdala. This suggests that the aPFC treats sequences of places, valence activations, and internal affective states the way the sensory neocortex treats sequences of sensory information.
...
Humans and, indeed, all mammals (and some other animals that independently evolved simulation) sometimes pause to simulate their options (model-based, goal-driven, system 2) and sometimes act automatically (model-free, habitual, system

Karl Friston also offers an explanation for the perplexing fact that some parts of the frontal cortex are missing the fourth layer of the neocortical column. What does layer four do? In the sensory cortex, layer four is where raw sensory input flows into a neocortical column. Layer four is speculated to have the role of pushing the rest of the neocortical column to render a simulation that best matches its incoming sensory data (perception by inference).

Thus, the aPFC spends very little, if any, time trying to match its inferred intent to the behavior it sees, and so it has no need for a large, or even any, layer four.

Controlling ongoing behavior often also requires working memory—the maintenance of representations in the absence of any sensory cues. Many imagined paths and tasks involve waiting.
...
This process of ever-escalating deceptions and counter-deceptions reveals that both Rock and Belle were able to understand the other’s intent (“ Belle is trying to lead me away from the food,” “Rock is trying to trick me by looking away”), as well as understand that it is possible to manipulate the other’s beliefs (“ I can make Belle think I am not looking by pretending to be disinterested,” “I can make Rock think the food is in the wrong location by leading him in that direction”).
...
If mirror neurons were simply automatic mirrors, then they wouldn’t activate in the above cases where monkeys were not directly observing behaviors.
...
Setting up camp en route to a nearby popular fruit patch the night before requires anticipating the fact that you will be hungry tomorrow if you don’t take preemptive steps tonight to get to the food early.

He surprisingly found that being a frugivore seemed to explain the variation in relative brain size perhaps even better than the size of a primate’s social group.
...
Suddendorf may have been prescient in proposing that the general ability to model a dissociated mental state from your own can be repurposed for both theory of mind and anticipating future needs.

### Breakthrough #5: Speaking And The First Humans

#### 19: The Search For Human Uniqueness

If it were the case that humans wielded numerous intellectual capabilities that were entirely unique in kind, we would expect human brains to contain some unique neurological structures, some new wiring, some new systems. But the evidence is the opposite—there is no neurological structure found in the human brain that is not also found in the brain of our fellow apes, and evidence suggests that the human brain is literally just a scaled-up primate brain:

Human language differs from other forms of animal communication in two ways. First, no other known form of naturally occurring animal communication assigns declarative labels (otherwise known as symbols). A human teacher will point to an object or a behavior and assign it an arbitrary label: elephant, tree, running. In contrast, other animals’ communications are genetically hardwired and not assigned. Vervet monkey and chimpanzee gestures are almost identical across different groups that have no contact with each other. Monkeys and apes deprived of social contact still use the same gestures. In fact, these gestures are even shared across species of primates;

a reward: “When I hear sit, if I sit, I will get a treat” or “When I hear stay, if I stop moving, I will get a treat.” This is basic temporal difference learning—all vertebrates can do this. Declarative labeling, on the other hand, is a special feature of human language. A declarative label is one that assigns an object or behavior an arbitrary symbol—“ That is a cow,” “That is running,”—without any imperative at all.

The second way in which human language differs from other animal communication is that it contains grammar.
...
declarative labels and grammar, enables groups of brains to transfer their inner simulations to each other with an unprecedented degree of detail and flexibility.
...
Both Homo erectus and modern humans have a peculiar method of cooling down—while other mammals pant to lower their body temperature, modern humans sweat. These traits would have kept our ancestors’ bodies cool while they were trekking long distances in the hot savannah.

These changes are perplexing; with a bigger body and brain, Homo erectus would have needed more energy and thus stronger jaws and longer digestive tracts for consuming more food. In the 1990s, the primatologist Richard Wrangham proposed a theory to explain this: H. erectus must have invented cooking. When meat or vegetables are cooked, harder-to-digest cellular structures are broken down into more energy-rich chemicals.
...
Premature birthing and an extended period of childhood brain growth put pressure on H. erectus to change its parenting style.

Many paleoanthropologists believed this shifted Homo erectus group dynamics away from the promiscuous mating of chimpanzees to the (mostly) monogamous pair-bonding we see in today’s human societies.

One theory is that menopause evolved to push grandmothers to shift their focus from rearing their own children to supporting their children’s children.

The first words may have emerged from proto-conversations between parents and their children, perhaps for the simple purpose of ensuring the successful transmission of advanced tool manufacture.
...
The use of language for gossip plus the punishment of moral violators’ makes it possible to evolve high levels of altruism. Early humans born with extra altruistic instincts would have more successfully propagated in an environment that easily identified and punished cheaters and rewarded altruists.
...
Noam Chomsky, who argues that language initially evolved only as a trick for inner thinking.

#### 22: Chatgpt And The Window Into The Mind

This is a downside of reasoning by simulating—we fill in characters and scenes, often missing the true causal and statistical relationships between things.

Children do not simply listen to endless sequences of words until they can predict what comes next. They are shown an object, engage in a hardwired nonverbal mechanism of shared attention, and then the object is given a name. The foundation of language learning is not sequence learning but the tethering of symbols to components of a child’s already present inner simulation. A human brain, but not GPT-3, can check the answers to mathematical operations using mental simulation.
...
Even if LLMs correctly answer commonsense and theory-of-mind questions, it does not necessarily mean it reasons about these questions in the same way.

### Conclusion: The Sixth Breakthrough

Breakthrough #1 was steering: the breakthrough of navigating by categorizing stimuli into good and bad, and turning toward good things and away from bad things.

Breakthrough #2 was reinforcing: the breakthrough of learning to repeat behaviors that historically have led to positive valence and inhibit behaviors that have led to negative valence.

Breakthrough #3 was simulating: the breakthrough of mentally simulating stimuli and actions.

Breakthrough #4 was mentalizing: the breakthrough of modeling one’s own mind.
...
It can be hard to conceptualize just how young our fourteen-billion-year-old universe actually is.
...
Rebecca Gelernter and Mesa Schumacher
